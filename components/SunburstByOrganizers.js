...
class SunburstByOrganizers extends React.PureComponent {
  static propTypes = {
...
export default SunburstByOrganizers
  |> withPropsOnChange(["data"], ({ data }) => {
    const resultByOrganizers = {};
    _.forEach(data, item => {
      if (
        !item["標記用＿派發機關"] ||
      ...
      const value = _.get(resultByOrganizers, paths, 0);
      _.set(resultByOrganizers, paths, value + 1);
    });
    ...
    return {
      data: {
        color: "#FFFFFF",
        name: "資料開放現況",
        children: _.map(resultByOrganizers, (resultA, nameA) => ({
